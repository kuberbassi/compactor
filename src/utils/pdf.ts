...
/** Structured Markdown export, shared with the workspace. */
export const markdownToPdf = async (markdown: string, title?: string): Promise<Blob> => {
  const { compileMarkdownPdf } = await import('./markdownPdf');
  return (await compileMarkdownPdf(markdown, title)).blob;
};

export const textToPdf = async (text: string, title?: string): Promise<Blob> => {
  const { compilePlainTextPdf } = await import('./markdownPdf');
  return compilePlainTextPdf(text, title);
};
